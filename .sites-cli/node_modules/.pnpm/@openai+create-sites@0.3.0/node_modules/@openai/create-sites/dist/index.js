...
function parseAddOns(value) {
    ...
    for (const name of requested) {
        if (!addOns.some((addOn) => addOn.name === name)) {
            throw new InvalidArgumentError(`Unknown add-on ${JSON.stringify(name)}. Available add-ons: ${addOns.map((addOn) => addOn.name).join(', ')}.`);
        }
    }
    return addOns
...
async function createSite(directory, options) {
    ...
    const manager = options.packageManager ??
        packageManagers.find((name) => process.env.npm_config_user_agent?.startsWith(`${name}/`)) ??
        'npm';
    const destination = resolve(process.cwd(), directory ?? '.');
    try {
        const entries = await readdir(destination);
        const existing = entries.filter((entry) => !allowedExistingEntries.has(entry));
        if (existing.length > 0) {
            ...
            throw new Error(`Target contains existing files: ${names}${remaining > 0 ? ` and ${remaining} more` : ''}.`);
        }
    }
    catch (error) {
    ...
    const hosting = JSON.parse(await readFile(join(base, '.openai', 'hosting.json'), 'utf8'));
    ...
    await rename(join(destination, '_gitignore'), join(destination, '.gitignore'));
    ...
    process.stdout.write(`Add-ons: ${selected.length ? selected.join(', ') : 'none'}.\n`);
    if (!options.install) {
...
async function main() {
    ...
    await new Command()
        ...
        .addOption(new Option('--package-manager <name>', 'Package manager').choices(packageManagers))
        .action(createSite)
        .parseAsync();
}
main().catch((error) => {
